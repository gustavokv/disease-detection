...
IMG_SIZE = (224, 224)  # Ideal para MobileNetV3
BATCH_SIZE = 16
CHANNELS = 3
...
def load_and_prepare_dataset(data_dir, img_size, batch_size):
    ...
    def preprocess(image, label):
        image = applications.mobilenet_v3.preprocess_input(image)
...
def evaluate_model(model, val_ds, class_names):
    ...
    plt.title("Matriz de Confusão - MobileNetV3")
    plt.tight_layout()
    plt.show()
...
history_fine = model.fit(
    ...
    callbacks=[
        ...
        keras.callbacks.ModelCheckpoint('/content/drive/MyDrive/saved_models/mobilenetv3_db1_corn_classifier.keras', monitor='val_accuracy', save_best_only=True, mode='max')
    ]
)
...
plot_history(DummyHistory(combined_history), 'Treinamento Completo - MobileNetV3')
